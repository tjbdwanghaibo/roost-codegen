...
type HeroDao struct {
	id                int64
	tracker           checkpoint.DirtyTracker   // ① dirty：字段位掩码追踪
	persistPatchSet   map[string]any            // ③ patch：待落库的路径级更新
...
**① dirty——每个字段一个掩码位，按 persist/sync 意图打标。** setter 先比较再标脏；`MarshalSync(mask)` / `MarshalPersistPatch(mask)` 只序列化掩码命中的字段：
...
func (d *HeroDao) markExpDirty() {
	d.tracker.MarkScope(checkpoint.DirtyPersist, heroDaoFieldExp)   // dao:"persist"
}
func (d *HeroDao) markCacheDirty() {
	d.tracker.MarkScope(checkpoint.DirtySync, heroDaoFieldCache)    // dao:"nopersist,sync"
}
```
...
**③ patch——map 的键级修改被记录为 MongoDB 路径级 `$set`/`$unset`**，而不是整字段重写：

```go
func (d *HeroDao) markItemsKeyDirty(key int64, val int32) {
	if path, ok := checkpoint.MapPatchPath("items", key); ok {
		d.recordPersistPatchSet("items", path, val)   // e.g. $set "items.1001"
	} else {
		d.recordPersistPatchFull("items")             // key 不能安全入路径时退回整字段
	}
	d.markItemsDirty()
}
```

`MarshalPersistPatch(mask)` 汇总为 `checkpoint.PersistPatch{Set, Unset, FullData}`，同时携带全量 `FullData` 供插入和兜底。

嵌套 struct（如 `EquipInfo`）生成到 `gen_<name>_nested.go`，内嵌 `checkpoint.DirtyHook`；`Init()` 把它的修改通知接到父 DAO 的对应字段掩码上，深层修改照样标脏。
...
- `coll=`、`db=` 必填；`dbscope` 默认 `global`，`sid` 表示按服分库（`checkpoint.DatabaseScopeServer`）。
