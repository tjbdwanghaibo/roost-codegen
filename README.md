...
>
> 生成器从 **roost-core v1.16.0** 起位于 `roost-core/codegen/`，demo 模板位于 `roost-core/demo/`。
> 本仓库不再接受改动，最后一个版本是 **v1.15.32**；旧 tag 永远可用。
>
> 安装 CLI：`go install github.com/tjbdwanghaibo/roost-core/codegen/cmd/roost@latest`
>
> 升级已有工程：`roost project upgrade --consolidate`（先 `--dry-run` 预览）。方案见
